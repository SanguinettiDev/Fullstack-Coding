  ...
  const loadPlantations = async () => {
    ...
    plantations.forEach((plantation) => {
      ...
      row.innerHTML = `
                ...
                <td>${
                  plantation.responsible ? plantation.responsible.name : "N/A"
                }</td>
                <td>
                    <button class="editPlantationBtn" data-id="${
                      plantation._id
                    }">Editar</button>
                    <button class="deletePlantationBtn" data-id="${
                      plantation._id
                    }">Deletar</button>
                </td>
            `;
    ...
    document.querySelectorAll(".editPlantationBtn").forEach((button) => {
      button.addEventListener("click", (e) =>
        openEditPlantationModal(e.target.dataset.id)
      );
    });

    document.querySelectorAll(".deletePlantationBtn").forEach((button) => {
      button.addEventListener("click", (e) =>
        deletePlantation(e.target.dataset.id)
      );
    });
  };
